#!/usr/bin/env python3
"""
sync-timing-batch2.py — Sync Batch 2 startFrames using pydub silence detection.

No Whisper, no network. Works fully offline.
Splits each MP3 on silence gaps, maps chunks to phrases in order,
writes corrected startFrame + totalFrames into scripts-batch2.ts.
...
BATCH2_TS = os.path.join(SRC, 'src', 'scripts-batch2.ts')
FPS = 30
...
def get_chunk_starts(audio_path, phrase_count):
    ...
    while len(chunks) > phrase_count:
        # Find the two closest chunks and merge them
...
def update_batch2_ts(composition_id, new_frames, total_frames, content):
    ...
    if not comp_match:
        print(f"  ⚠️  Could not find '{composition_id}' in scripts-batch2.ts")
        return content
...
def main():
    print("\n🎙  Batch 2 Timing Sync (pydub silence detection)\n" + "─" * 48)

    with open(BATCH2_TS, 'r', encoding='utf-8') as f:
        content = f.read()
    ...
    for video in VIDEOS:
        ...
        content = update_batch2_ts(comp, new_frames, total_frames, content)

    with open(BATCH2_TS, 'w', encoding='utf-8') as f:
        f.write(content)

    print("\n✅  scripts-batch2.ts updated with synced timing.")
    print("Next: npx remotion render for each batch 2 composition.\n")
